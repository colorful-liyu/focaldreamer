...
class Decoder(torch.nn.Module):
    ...
    def pre_train_ellipsoid(self, it, scene_and_vertices):
        ...
        points_surface = scene_and_vertices[1].astype(np.float32)  # 5000 points
        points_surface_disturbed = points_surface + np.random.normal(loc=0.0, scale=0.05, size=points_surface.shape).astype(np.float32)    # 5000 points 
        point_rand = (np.random.rand(3000,3).astype(np.float32)-0.5)* self.mesh_scale  # 3000 points
