...
from dataset.dataset_mesh_temp import DatasetMesh #changed
from dataset.dataset_mesh_temp import get_camera_params #changed

# Import topology / geometry trainers
from geometry.dmtet2 import DMTetGeometry #changed
...
def prepare_batch(target, background= 'black'):
    ...
    if 'img' in target:
        # print(target['img'][..., 3:4])
        target['img'] = torch.cat((torch.lerp(target['background'], target['img'][..., 0:3], target['img'][..., 3:4]), target['img'][..., 3:4]), dim=-1) 
        # print(target['img'])
        # util.save_image(FLAGS.out_dir + '/' + ('img_ref_%s_%06d.png' % (FLAGS.mode, 168)), target['img'][0].detach().cpu().numpy())
    return target
...
def validate_itr(glctx, target, geometry, opt_material, lgt, FLAGS):
    ...
    with torch.no_grad():
        ...
        randomint = random.randint(0, buffers['shaded'].shape[0]-1)
        result_dict['shaded'] =  buffers['shaded'][randomint, ..., 0:3]
        result_dict['shaded'] = util.rgb_to_srgb(result_dict['shaded'])
        result_dict['mask'] = (buffers['shaded'][randomint, ..., 3:4])
        result_dict['ref'] = util.rgb_to_srgb(target['img'][...,0:3])[randomint]
        result_image = torch.cat([result_dict['shaded'], result_dict['ref']], axis=1)
        if FLAGS.display is not None :
            ...
            for layer in FLAGS.display:
                if 'latlong' in layer and layer['latlong']:
                    ...
                    result_dict['relight'] = util.rgb_to_srgb(img[..., 0:3])[randomint]
                    ...
                    if layer['bsdf'] == 'kd':
                        result_dict[layer['bsdf']] = util.rgb_to_srgb(buffers['shaded'][randomint, ..., 0:3])  
                    elif layer['bsdf'] == 'normal':
                        result_dict[layer['bsdf']] = (buffers['shaded'][randomint, ..., 0:3] + 1) * 0.5
                    else:
                        result_dict[layer['bsdf']] = buffers['shaded'][randomint, ..., 0:3]
...
class Trainer(torch.nn.Module):
    def __init__(self, glctx, geometry, lgt, mat, optimize_geometry, optimize_light, FLAGS, guidance):
        ...
        self.image_loss_fn = createLoss(FLAGS)
    ...
    def forward(self, target, it, if_normal):
        ...
        return self.geometry.tick(glctx, target, self.light, self.material, it , if_normal, self.guidance, self.FLAGS.mode, self.if_flip_the_normal, self.image_loss_fn)
...
def optimize_mesh(
    ...
    def lr_schedule(iter, fraction):
        return max(0.0, 10**(-(iter)*(1.0/FLAGS.iter))) # Exponential falloff from [1.0, 0.1] over epochs. 
    ...
    if optimize_geometry: 
        optimizer_mesh = torch.optim.AdamW(trainer_noddp.geo_params, lr=learning_rate_pos, betas=(0.9, 0.99), eps=1e-15)
        scheduler_mesh = torch.optim.lr_scheduler.LambdaLR(optimizer_mesh, lr_lambda=lambda x: lr_schedule(x, 0.9)) 
    optimizer = torch.optim.AdamW(trainer_noddp.params, lr=learning_rate_mat, betas=(0.9, 0.99), eps=1e-15)
    ...
    for it, target in enumerate(dataloader_train):
        ...
        if FLAGS.local_rank == 0:
            ...
            if  save_image:#v_it
                ...
                #ref_mesh image save 
                # util.save_image(FLAGS.out_dir + '/' + ('img_ref_%s_%06d.png' % (FLAGS.mode, img_cnt)), target['img'][0].detach().cpu().numpy())
...
if __name__ == "__main__":
    ...
    if FLAGS.mode == 'appearance_modeling' and FLAGS.base_mesh is not None:
        if FLAGS.learn_light:
            FLAGS.lgt = light.create_trainable_env_rnd(512, scale=0.0, bias=1)
            lgt = FLAGS.lgt
        else:
            FLAGS.lgt = light.load_env(FLAGS.envmap, scale=FLAGS.env_scale)
            lgt = FLAGS.lgt
    else:
        FLAGS.lgt = None
        lgt = None
