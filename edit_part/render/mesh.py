...
######################################################################################
class Mesh:
    def __init__(self, v_pos=None, t_pos_idx=None, v_nrm=None, t_nrm_idx=None, v_tex=None, t_tex_idx=None, v_tng=None, t_tng_idx=None, len_editable_part=None, material=None, base=None):
        self.v_pos = v_pos           #顶点位置   [5344,3]
        self.v_nrm = v_nrm           #顶点法线   [5344,3]
        self.v_tex = v_tex           #顶点uv坐标 [5647,2] 数量比顶点数量多，因为一个顶点可能参与多个三角形，在不同三角形中有不同的纹理坐标
        self.v_tng = v_tng           #顶点切线
        self.t_pos_idx = t_pos_idx   #三角面片，关于三个顶点的索引 [10688,3]
        self.t_nrm_idx = t_nrm_idx   #三角面片中的关于三个顶点的法线索引(这个顶点对应着哪个法线)   [10688,3]                  
        self.t_tex_idx = t_tex_idx   #三角面片中每个顶点对应的uv坐标的索引(这个顶点对应着哪个uv坐标)    [10688,3]
        self.t_tng_idx = t_tng_idx
        self.material = material
        self.len_editable_part = len_editable_part
    ...
    def copy_none(self, other):
        ...
        if self.len_editable_part is None:
            self.len_editable_part = other.len_editable_part
    ...
    def clone(self):
        ...
        if out.len_editable_part is not None:
            out.len_editable_part = out.len_editable_part
        return out
