...
def shade(
    ...
    if bsdf == 'texc_no_preserve':
        editable_mask = torch.ones_like(editable_mask)
    if 'kd_ks_normal' in material:
        # Combined texture, used for MLPs because lookups are expensive
        all_tex_jitter = material['kd_ks_normal'].edit_sample(gb_pos + torch.normal(mean=0, std=0.01, size=gb_pos.shape, device="cuda"), editable_mask)
        all_tex = material['kd_ks_normal'].edit_sample(gb_pos, editable_mask)
...
def render_layer(
    ...
    buffers = shade(gb_pos, gb_geometric_normal, gb_normal, gb_tangent, gb_texc, gb_texc_deriv, 
        view_pos, lgt, mesh.material, bsdf,if_normal,normal_rotate, mode, if_flip_the_normal, editable_mask)
...
def render_mesh(
    ...
    # generate the mask with mesh-merged mesh
    with torch.no_grad():
        v_pos_clip_formask = ru.xfm_points(mesh_formask.v_pos[None, ...], mtx_in)
        layers = []
        with dr.DepthPeeler(ctx, v_pos_clip_formask, mesh_formask.t_pos_idx.int(), full_res) as peeler:
        ...
        if spp > 1 and msaa:
            ...
            rast_out_s = rast
        editable_mask = rast_out_s[:, :, :, -1].unsqueeze(-1).clone().detach() < mesh_formask.len_editable_part # True is editable, False is not editable
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), full_res) as peeler:
        for _ in range(num_layers):
            rast, db = peeler.rasterize_next_layer()
            layers += [(render_layer(rast, db, mesh, view_pos, lgt, resolution, spp, msaa, bsdf, if_normal, normal_rotate, mode, if_flip_the_normal, editable_mask ), rast)]
    ...
    out_buffers['editable_mask'] = editable_mask
...
#  Render UVs
# ==============================================================================================
def render_uv(ctx, mesh, mesh_formask, resolution, mlp_texture, uv_padding_block):

    # for mask
    # with torch.no_grad():
    #     uv_clip_formask = mesh_formask.v_tex[None, ...]*2.0 - 1.0
    #     uv_clip4_formask = torch.cat((uv_clip_formask, torch.zeros_like(uv_clip_formask[...,0:1]), torch.ones_like(uv_clip_formask[...,0:1])), dim = -1)
    #     rast_formask, _ = dr.rasterize(ctx, uv_clip4_formask, mesh_formask.t_tex_idx.int(), resolution)
    #     editable_mask = rast_formask[:, :, :, -1].unsqueeze(-1).clone().detach() < mesh_formask.len_editable_part # True is editable, False is not editable
    ...
    rast_formask, _ = dr.rasterize(ctx, uv_clip4, mesh_formask.t_tex_idx.int(), resolution)
    editable_mask = rast_formask[:, :, :, -1].unsqueeze(-1).clone().detach() < mesh_formask.len_editable_part
    ...
    all_tex = mlp_texture.edit_sample(gb_pos, editable_mask)
    assert all_tex.shape[-1] == 9 or all_tex.shape[-1] == 10, "Combined kd_ks_normal must be 9 or 10 channels"
