...
if __name__ == "__main__":
    ...
    parser.add_argument('--ft_ckpt', type=str, default='')
    ...
    parser.add_argument('--coincide_w', type= float, default=10)
    ...
    parser.add_argument("--base_mesh_rotate_y", nargs='+', type= int, help="rotation of the initial shape on the y-axis")
    ...
    if FLAGS.mode == 'geometry_modeling' :   
        ...
        # 创建一个空的state_dict字典，适应ref_encoder网络的键和权重
        ref_mat_state_dict = {}
        # 遍历预训练权重文件的state_dict的键，并适应为ref_encoder网络的键
        for key, value in state['material'].items():
            new_key = key.replace('kd_ks_normal.encoder.', 'kd_ks_normal.ref_encoder.')
            new_key = new_key.replace('kd_ks_normal.net.', 'kd_ks_normal.ref_net.')
            ref_mat_state_dict[new_key] = value
        mat.load_state_dict(ref_mat_state_dict, strict=False)
        ...
        if FLAGS.ft_ckpt != '':
            state2 = torch.load(FLAGS.ft_ckpt)
            geometry.load_state_dict(state2['geometry'])
        ...
        #     if FLAGS.local_rank == 0:
        #         os.makedirs("demo/dmtet_mesh", exist_ok=True)
        #         obj.write_obj("demo/dmtet_mesh/", base_mesh)
        ...
        with torch.no_grad():
            ...
            # # !!!!!!!!! init learnable mat with ref mat
            # state = torch.load(os.path.join(FLAGS.ref_mat))
            # mat.load_state_dict(state['material'], strict=False)
