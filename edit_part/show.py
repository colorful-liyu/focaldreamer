...
@torch.no_grad()
def xatlas_uvmap(glctx, geometry, mat, FLAGS, get_edit=False):
    sdf_mesh, mesh_mesh, edit_mesh = geometry.getMesh_sdfmeshmerge(mat)

    mesh_formask = xatlas_operator(mesh_mesh)
    if get_edit:
        new_mesh = xatlas_operator(edit_mesh)
        mesh_formask.len_editable_part = 1000000000 # a extrimely large number
    else:
        new_mesh = xatlas_operator(sdf_mesh)

    # mask, kd, ks, normal = render.render_uv(glctx, new_mesh, mesh_formask, FLAGS.texture_res, new_mesh.material['kd_ks_normal'])
    mask, kd, ks, normal = render.render_uv(glctx, new_mesh, mesh_formask, FLAGS.texture_res, new_mesh.material['kd_ks_normal'], FLAGS.uv_padding_block)
...
###############################################################################
# @torch.no_grad()  
def validate_itr(glctx, target, geometry, opt_material, lgt, FLAGS, render_mode='all'):
    result_dict = {}
    with torch.no_grad():
        ...
        if render_mode=='only_edit_geo':
            buffers = geometry.onlyedit_render(glctx, target, lgt, opt_material, mode=FLAGS.mode)
        elif render_mode=='origin':
            buffers = geometry.origin_render(glctx, target, lgt, opt_material, mode=FLAGS.mode)
        elif render_mode=='editable_texture':
            buffers = geometry.editable_texture_render(glctx, target, lgt, opt_material, mode=FLAGS.mode)
        else:
            buffers = geometry.render(glctx, target, lgt, opt_material, mode=FLAGS.mode)
        result_dict['shaded'] =  buffers['shaded'][0, ...]
        ...
        if FLAGS.display is not None :
            ...
            for layer in FLAGS.display:
                if 'latlong' in layer and layer['latlong']:
                    ...
                    if render_mode=='only_edit_geo':
                        buffers  = geometry.onlyedit_render(glctx, target, lgt, opt_material, bsdf=layer['bsdf'])
                    elif render_mode=='origin':
                        buffers = geometry.origin_render(glctx, target, lgt, opt_material, bsdf=layer['bsdf'])
                    elif render_mode=='editable_texture':
                        buffers = geometry.editable_texture_render(glctx, target, lgt, opt_material, bsdf=layer['bsdf'])
                    else:
                    ...
                    if layer['bsdf'] == 'kd':
                        ...
                        result_dict[layer['bsdf']] = torch.cat(((buffers['shaded'][0, ..., 0:3] + 1) * 0.5,buffers['shaded'][0, ..., 3].unsqueeze(-1)), dim=-1)
                    elif layer['bsdf'] == 'texc_no_preserve':
                        result_dict[layer['bsdf']] = util.rgb_to_srgb(buffers['shaded'][0, ..., 0:3])  
                    else:
                        result_dict[layer['bsdf']] = buffers['shaded'][0, ..., 0:3]

        return result_dict['shaded'], result_dict
...
def save_gif(dir,fps):
    ...
    frames[0].save(os.path.join(dir, 'eval.gif'), save_all=True, append_images=frames[1:], loop=0, disposal=2, duration=1000/fps)
        # frames.append(imageio.v3.imread(img, plugin="pillow", mode="RGBA"))
    # imageio.v3.imwrite(os.path.join(dir, 'eval.gif'), frames, plugin="pillow", mode="RGBA", duration=1000/fps, loop=0, transparency=0)
    # imageio.mimsave(os.path.join(dir, 'eval.gif'),frames,'GIF',duration=1/fps)



@torch.no_grad()     
def validate(glctx, geometry, opt_material, lgt, dataset_validate, out_dir, FLAGS, render_mode='all'):
    ...
    for it, target in enumerate(dataloader_validate):
        ...
        result_image, result_dict = validate_itr(glctx, target, geometry, opt_material, lgt, FLAGS, render_mode)
...
class Trainer(torch.nn.Module):
    ...
    def forward(self, target, it,if_normal):
        if self.FLAGS.mode == 'appearance_modeling':
...
def optimize_mesh(
    ...
    trainer_noddp = Trainer(glctx, geometry, lgt, opt_material, optimize_geometry, optimize_light, FLAGS, guidance)

    if FLAGS.multi_gpu: 
        model = trainer_noddp.cuda()
    ...
    if optimize_geometry: 
        optimizer_mesh = torch.optim.AdamW(trainer_noddp.geo_params, lr=0.001, betas=(0.9, 0.99), eps=1e-15)
    optimizer = torch.optim.AdamW(trainer_noddp.params, lr=0.01, betas=(0.9, 0.99), eps=1e-15)
    ...
    for it, target in enumerate(dataloader_train):
            ...
            if save_video:
                with torch.no_grad():
                    ...
                    if FLAGS.mode =='geometry_modeling':
                        ...
                        buffers  = geometry.render(glctx, params, lgt, opt_material, mode=FLAGS.mode, bsdf='pbr')
        ...
        with torch.cuda.amp.autocast(enabled= True):
            sds_loss,img_loss, reg_loss = model(target, it, if_normal)
        ...
        scaler.scale(sds_loss+total_loss).backward()
        ...
        if it > 0 and it % 1000 == 0 and FLAGS.local_rank == 0:
            # save model
...
if __name__ == "__main__":
    ...
    parser.add_argument('-rm', '--ref_mesh', type=str, default='data/horse.obj')
    parser.add_argument('-bm', '--base-mesh', type=str, default='data/sphere.obj')
    parser.add_argument("--sdf_softmerge", action='store_true', default=False , help="Soft sdf merge.")
    parser.add_argument("--pie_softmerge", action='store_true', default=False , help="Soft pie merge.")
    parser.add_argument('--geo_ckpt', type=str, default='')
    parser.add_argument('--ref_mat', type=str, default='')
    parser.add_argument('--base_mesh_scale', nargs='+', type=lambda x: [int(item) for item in x.split(',')]) # 1,2,3 4,5,6 7,8,9 # x是红线，y是绿线，z是蓝线
    parser.add_argument('--base_mesh_offset', nargs='+', type=lambda x: [int(item) for item in x.split(',')]) # 1,2,3 4,5,6 7,8,9
    parser.add_argument('--ref_mesh_scale', nargs=3, type=float, default=[1, 1, 1])
    parser.add_argument('--ref_mesh_offset', nargs=3, type=float, default=[0, 0, 0])
    parser.add_argument('--region_w', type= float, default=1000, help="W for shape in local regions.")
    parser.add_argument('--coincide_w', type= float, default=100)
    parser.add_argument('--consb_w', type= float, default=0)
    parser.add_argument('--consg_w', type= float, default=0)
    ...
    parser.add_argument("--base_mesh_rotate_x", nargs='+', type= int, help="rotation of the initial shape on the x-axis")
    parser.add_argument("--base_mesh_rotate_z", nargs='+', type= int, help="rotation of the initial shape on the z-axis")
    ...
    assert FLAGS.mode == 'appearance_modeling'
    # ==============================================================================================
    ...
    with torch.no_grad():
        state = torch.load(os.path.join(FLAGS.geo_ckpt))
        mat.load_state_dict(state['material'])
        mat['kd_ks_normal'].close_grad_refnet()
        geometry.init_ref_shape(FLAGS, mat)
        geometry.load_state_dict(state['geometry'])
        # state = torch.load(os.path.join(FLAGS.ref_mat))
        # mat.load_state_dict(state['material'], strict=False)
    ...
    # ==============================================================================================
    #  Validate   [render_mode: 'only_edit_geo', 'origin', 'editable_texture', 'all']
    # ==============================================================================================
    if FLAGS.validate and FLAGS.local_rank == 0:
        validate(glctx, geometry, mat, lgt, dataset_gif, os.path.join(FLAGS.out_dir, "validate"), FLAGS, 'all')
        validate(glctx, geometry, mat, lgt, dataset_gif, os.path.join(FLAGS.out_dir, "validate_onlyeditgeo"), FLAGS, 'only_edit_geo')
        validate(glctx, geometry, mat, lgt, dataset_gif, os.path.join(FLAGS.out_dir, "validate_baseshape"), FLAGS, 'origin')
        validate(glctx, geometry, mat, lgt, dataset_gif, os.path.join(FLAGS.out_dir, "validate_editable_texture"), FLAGS, 'editable_texture')
    base_mesh = xatlas_uvmap(glctx, geometry, mat, FLAGS)
    base_edit_mesh = xatlas_uvmap(glctx, geometry, mat, FLAGS, True)
    ...
    if FLAGS.local_rank == 0:
        ...
        os.makedirs(os.path.join(FLAGS.out_dir, "dmtet_edit_mesh"), exist_ok=True)
        obj.write_obj(os.path.join(FLAGS.out_dir, "dmtet_edit_mesh/"), base_edit_mesh)
